...
class BookRepositoryImpl(
    ...
    override fun getBookById(id: String): Flow<Resource<ServiceDetail>> = flow<Resource<ServiceDetail>> {
        emit(Resource.Loading)
        when (val response = service.getBookById(id)){
            is NetworkResponse.Success -> {
                val result = response.body.data?.toDomain(ServiceType.BOOK)
                if (result == null){
...
//                emit(Resource.Success(result))
    ...
    override fun getRecommendationBook(
        ...
        when (val response = service.getRecommendationBook(id,authToken)){
            is NetworkResponse.Success -> {
                val result = response.body.data?.toDomains(type = ServiceType.BOOK) ?: emptyList()
                emit(Resource.Success(result))
            }
            is NetworkResponse.Error -> {
                val message = response.error?.message ?: response.error?.message
    ...
    override fun getBook(): Flow<PagingData<ServiceItem>> {
        return Pager(
            ...
            pagingSourceFactory = { ServicePagingSource(service = service, type = ServiceType.BOOK)}
        ).flow
    }
    ...
    override fun searchBookNeeds(
        ...
        return Pager(
            config = PagingConfig(pageSize = ServicePagingSource.DEFAULT_SIZE),
            pagingSourceFactory = { ServicePagingSource(service = service, name = name, field = field, fees = fees, type = ServiceType.BOOK)}
        ).flow
    }
