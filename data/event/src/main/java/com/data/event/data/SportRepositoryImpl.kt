...
class SportRepositoryImpl(
    ...
    override fun getSportById(id: String): Flow<Resource<ServiceDetail>> = flow<Resource<ServiceDetail>> {
        emit(Resource.Loading)
        when (val response = service.getSportById(id)){
    ...
    override fun getRecommendationSport(
        ...
        when (val response = service.getRecommendationSport(id,authToken)){
            is NetworkResponse.Success -> {
                val result = response.body.data?.toDomains(type = ServiceType.SPORT) ?: emptyList()
                emit(Resource.Success(result))
            }
            is NetworkResponse.Error -> {
                val message = response.error?.message ?: response.error?.message
    ...
    override fun getSport(): Flow<PagingData<ServiceItem>> {
        return Pager(
            ...
            pagingSourceFactory = { ServicePagingSource(service = service, type = ServiceType.SPORT)}
        ).flow
    }
    ...
    override fun searchSportNeeds(
        ...
        return Pager(
            config = PagingConfig(pageSize = ServicePagingSource.DEFAULT_SIZE),
            pagingSourceFactory = { ServicePagingSource(service = service, name = name, field = field, fees = fees, type = ServiceType.SPORT)}
        ).flow
    }
